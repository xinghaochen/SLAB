...
def main():
    ...
    # transfer_to_onnx(model, './onnx/relu_q_linear.onnx')
    ...
    for e in range(10):
        data_loader_train.sampler.set_epoch(e+300)
        update_model(config, data_loader_train, model, logger)

        if dist.get_rank() == 0 and e % config.SAVE_FREQ == 0 :
            save_checkpoint_new(config, e+300, model_without_ddp, max_accuracy, optimizer, lr_scheduler, logger)
...
@torch.no_grad()
def update_model(config, data_loader, model, logger):
...
def transfer_to_onnx(model, out_name):
    model.eval()
    ...
    dummy_input = torch.rand(1, 3, 224, 224).cuda()
    torch.onnx.export(model, dummy_input, out_name, input_names=input_names, output_names=output_names,
                      dynamic_axes={'input': [0], 'output': [0]}, opset_version=12)
    return
